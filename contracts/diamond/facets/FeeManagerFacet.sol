...
contract FeeManagerFacet is IFeeManager, OnlySelf {
    ...
    function chargeOpenFee(address token, uint256 openFee, uint24 broker) external onlySelf override returns (uint24) {
        (uint24 brokerId, uint256 brokerAmount, uint256 daoAmount, uint256 lpPoolAmount) = LibFeeManager.chargeFee(token, openFee, broker);
        emit OpenFee(token, openFee, daoAmount, brokerId, brokerAmount, lpPoolAmount);
        return brokerId;
    }

    function chargePredictionOpenFee(address token, uint256 openFee, uint24 broker) external onlySelf override returns (uint24) {
        (uint24 brokerId, uint256 brokerAmount, uint256 daoAmount, uint256 lpPoolAmount) = LibFeeManager.chargeFee(token, openFee, broker);
        emit PredictionOpenFee(token, openFee, daoAmount, brokerId, brokerAmount, lpPoolAmount);
        return brokerId;
    }

    function chargeCloseFee(address token, uint256 closeFee, uint24 broker) external onlySelf override {
        (uint24 brokerId, uint256 brokerAmount, uint256 daoAmount, uint256 lpPoolAmount) = LibFeeManager.chargeFee(token, closeFee, broker);
        emit CloseFee(token, closeFee, daoAmount, brokerId, brokerAmount, lpPoolAmount);
    }

    function chargePredictionCloseFee(address token, uint256 closeFee, uint24 broker) external onlySelf override {
        (uint24 brokerId, uint256 brokerAmount, uint256 daoAmount, uint256 lpPoolAmount) = LibFeeManager.chargeFee(token, closeFee, broker);
        emit PredictionCloseFee(token, closeFee, daoAmount, brokerId, brokerAmount, lpPoolAmount);
    }
