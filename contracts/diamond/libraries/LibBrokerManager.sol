...
library LibBrokerManager {
    ...
    bytes32 constant BROKER_MANAGER_STORAGE_POSITION = keccak256("broker.manager.storage");
    ...
    event UpdateBrokerCommissionP(uint24 indexed id, uint16 commissionP, uint16 daoShareP, uint16 lpPoolP);
    ...
    function addBroker(
        uint24 id, uint16 commissionP, uint16 daoShareP, uint16 lpPoolP,
        ...
        Broker memory b = Broker(
            name, url, receiver, id, uint24(bms.brokerIds.length), commissionP, daoShareP, lpPoolP
        );
        bms.brokers[id] = b;
    ...
    function updateBrokerCommissionP(uint24 id, uint16 commissionP, uint16 daoShareP, uint16 lpPoolP) internal {
        BrokerManagerStorage storage bms = brokerManagerStorage();
        Broker storage b = _checkBrokerExist(bms, id);
        b.commissionP = commissionP;
        b.daoShareP = daoShareP;
        b.lpPoolP = lpPoolP;
        emit UpdateBrokerCommissionP(id, commissionP, daoShareP, lpPoolP);
    }
    ...
    function updateBrokerCommission(
    ...
    ) internal returns (uint256 commission, uint24 brokerId, uint256 daoAmount, uint256 lpPoolAmount){
        BrokerManagerStorage storage bms = brokerManagerStorage();
        ...
        return (commission, b.id, feeAmount * b.daoShareP / 1e4, feeAmount * b.lpPoolP / 1e4);
    }
}
